...
public class SkillContentValidator {
    ...
    public static List<SkillItemDTO> validate(List<SkillItemDTO> contentList) {
        if (contentList == null || contentList.isEmpty()) {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR.getCode(), "技能内容不能为空");
        }
        ...
        for (int i = 0; i < contentList.size(); i++) {
            ...
            if (item == null) {
                throw new BusinessException(ErrorCode.VALIDATION_ERROR.getCode(), path + " 不能为空");
            }
            if (item.getType() == null || item.getType().trim().isEmpty()) {
                throw new BusinessException(ErrorCode.VALIDATION_ERROR.getCode(), path + ".type 不能为空");
            }
            if (item.getContent() == null || item.getContent().isEmpty()) {
                throw new BusinessException(ErrorCode.VALIDATION_ERROR.getCode(), path + ".content 不能为空");
            }
