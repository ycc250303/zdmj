...
public interface AIService {
         ...
         * @param onComplete 完成时的回调函数（接收完整内容和元数据）
         * @param onError    错误时的回调函数
         * @param onMetadata 元数据回调函数（接收token使用情况、模型信息等）
         * @return SSE流式事件
         */
        Flux<ServerSentEvent<String>> streamChat(Prompt prompt, String messageId, String model,
                        java.util.function.Consumer<String> onChunk,
                        java.util.function.BiConsumer<String, java.util.Map<String, Object>> onComplete,
                        java.util.function.Consumer<Throwable> onError,
                        java.util.function.Consumer<java.util.Map<String, Object>> onMetadata);
