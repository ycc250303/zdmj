...
四维必须按本维考察点独立打分；禁止因技能关键词命中多而抬升 basic / professionalQuality / developmentPotential。某维岗位侧未写明时，按软件测试校招常规门槛评估，学生侧无场景证据则该维不超过 59。测试岗尤其不可用「会工具」替代素养维的质量意识打分。

## basic（基础要求）
对照学历、测试相关资质/证书、地点/实习等硬门槛。
- 90~100：硬门槛全部满足，且有测试相关课程、证书或竞赛等可核验证据；
...
对照测试理论与用例设计、接口测试、功能测试流程、自动化基础、SQL 与日志分析。
...
对照质量与风险意识、跨团队沟通（研发/产品/运维）、缺陷描述与推动修复、抗压与细致度。
- 90~100：有缺陷闭环（描述清晰、复现步骤、推动修复、回归），可追问到具体质量风险判断；
- 75~89：有跨团队沟通或缺陷提单证据，缺风险意识或高压场景；
- 60~74：仅有「细心负责」或课程测试作业等弱证据；
- 40~59：只有套话，无缺陷沟通或质量意识场景；
- 0~39：该维完全未体现，或与岗位强调的质量协作明显冲突。

## developmentPotential（发展潜力）
对照测试策略改进、自动化建设、学习能力、实习与项目中的质量产出。
- 90~100：有「测试对象—策略—缺陷/效率结果」闭环，能看出自动化或流程改进；
- 75~89：有完整测试实习或项目职责，缺量化或策略迭代；
- 60~74：以执行用例为主，学习证据多为工具罗列；
- 40~59：无成长斜率，描述停留在「参与测试」；
- 0~39：实践与学习证据严重缺失。
...
- `professionalQuality.gap` 必须点名缺失的场景类型（质量风险 / 缺陷沟通 / 跨团队推动 / 抗压细致）；
- `developmentPotential.gap` 必须点名缺失的成长证据（策略改进 / 自动化建设 / 质量量化结果）；
