...
**固定输出空数组 `[]`。** 奖项由系统按关键词切句后另一次模型调用提取，不要在本步填写。

# Rules

1. 原文没有的区块返回空数组 `[]` 或省略；不要填充占位符。
2. **日期必须逐条独立识别（educations / careers / projects 的 startDate、endDate）**：
   - 每条教育、实习/工作、项目**只使用该条原文里明确写出的时间**；识别不出则写 `null`。
   - **禁止套用其他模块或其他条目的时间**（例如不要用教育的入学/毕业时间填项目或实习，不要用实习周期填项目，也不要用相邻条目的日期补全本条）。
