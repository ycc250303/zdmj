...
1. **稳定性方案合理性**：识别并纠正不合理的运维/发布方案。
2. **业务场景融合**：描述必须遵循「运维场景 + 技术方案 + 稳定性结果」的模式。
3. **表达精炼度**：单条描述建议不超过两行。动词开头，删除冗余词汇。
4. **深度技术点**：优先挖掘 CI/CD、容器编排、可观测性、故障应急与自动化等高价值信息。
...
1. **projectExperienceScore（0-40，项目经验）**：DevOps/SRE 相关项目是否真实可追问；是否体现稳定性实践与故障处理；是否有量化结果（发布效率、故障时长、可用性、成本等）。
   - 31-40：项目深度强、证据充分、有量化结果
   - 18-30：有项目但深度不足或证据偏弱
   - 0-17：关键词堆砌、描述空泛或明显不匹配

2. **skillMatchScore（0-20，技能匹配）**：DevOps/SRE 校招核心能力覆盖（Linux/网络、CI/CD、Docker/K8s、可观测性、自动化脚本/IaC）；是否有项目或实习佐证。
...
4. **structureClarityScore（0-15，结构清晰度）**：简历层次是否清晰、模块划分是否易读；技术名词是否规范（CI/CD、K8s、SLO/SLA、MTTR、Prometheus、Grafana）；句式是否清晰，是否存在明显语病/堆词。
   - 12-15：结构清晰、排版规范
   - 6-11：可读但层次或规范一般
   - 0-5：混乱或大量不规范表达
...
3. 若缺少 DevOps/SRE 核心项（CI/CD 或容器编排 / 可观测性 / 项目实践）中的 2 项及以上，scoreDetail 五项之和不得高于 65。
