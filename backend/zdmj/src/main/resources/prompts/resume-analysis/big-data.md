...
1. **架构合理性**：识别并纠正不合理的数据链路方案。
2. **业务场景融合**：描述必须遵循「数据来源 + 处理流程 + 业务产出」的模式。
3. **表达精炼度**：单条描述建议不超过两行。动词开头，删除冗余词汇。
4. **深度技术点**：优先挖掘数仓建模、批流处理、任务治理、性能优化等高价值信息。
...
1. **projectExperienceScore（0-40，项目经验）**：大数据相关项目是否真实可追问；是否体现数据链路搭建与优化；是否有量化结果（时延、吞吐、资源成本、失败率等）。
   - 31-40：项目深度强、证据充分、有量化结果
   - 18-30：有项目但深度不足或证据偏弱
   - 0-17：关键词堆砌、描述空泛或明显不匹配

2. **skillMatchScore（0-20，技能匹配）**：大数据校招核心能力覆盖（SQL/ETL、Spark/Flink/Hive/Kafka 等组件、数仓建模、调度治理）；是否有项目或实习佐证。
...
4. **structureClarityScore（0-15，结构清晰度）**：简历层次是否清晰、模块划分是否易读；技术名词是否规范（ODS/DWD/DWS/ADS、Batch/Streaming、Checkpoint 等）；句式是否清晰，是否存在明显语病/堆词。
   - 12-15：结构清晰、排版规范
   - 6-11：可读但层次或规范一般
   - 0-5：混乱或大量不规范表达
...
3. 若缺少大数据核心项（SQL/数据处理 / 组件实践 / 项目实践）中的 2 项及以上，scoreDetail 五项之和不得高于 65。
