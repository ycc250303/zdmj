...
1. **测试策略合理性**：识别并纠正不合理的测试方案（如只写功能测试却声称覆盖全链路质量保障）。
2. **业务场景融合**：描述必须遵循「测试对象 + 测试策略 + 缺陷/质量结果」的模式。
3. **表达精炼度**：单条描述建议不超过两行。动词开头，删除冗余词汇。
4. **深度测试点**：优先挖掘用例设计、缺陷定位、自动化实践、质量指标等高价值信息。
...
1. **projectExperienceScore（0-40，项目经验）**：测试相关项目是否真实可追问；是否体现「发现问题-定位问题-推动修复」闭环；是否有量化结果（缺陷发现率、回归效率、线上故障下降等）。
   - 31-40：项目深度强、证据充分、有量化结果
   - 18-30：有项目但深度不足或证据偏弱
   - 0-17：关键词堆砌、描述空泛或明显不匹配

2. **skillMatchScore（0-20，技能匹配）**：软件测试校招核心能力覆盖（测试理论基础、接口测试、功能测试流程、自动化测试基础、SQL 与日志分析）；是否有项目或实习佐证。
...
4. **structureClarityScore（0-15，结构清晰度）**：简历层次是否清晰、模块划分是否易读；技术名词是否规范（Test Case、API、Selenium、Pytest、SQL、JMeter）；句式是否清晰，是否存在明显语病/堆词。
   - 12-15：结构清晰、排版规范
   - 6-11：可读但层次或规范一般
   - 0-5：混乱或大量不规范表达
...
3. 若缺少测试核心项（测试理论基础 / 测试实践项目 / 缺陷分析能力）中的 2 项及以上，scoreDetail 五项之和不得高于 65。
