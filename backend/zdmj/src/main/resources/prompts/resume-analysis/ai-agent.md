...
1. **方案合理性**：识别并纠正不合理的 AI 方案（如把简单 API 调用包装为复杂 Agent 系统）。
2. **业务场景融合**：描述必须遵循「业务场景 + 方案设计 + 效果结果」的模式。
3. **表达精炼度**：单条描述建议不超过两行。动词开头，删除冗余词汇。
4. **深度技术点**：优先挖掘 Prompt 迭代、工具编排、RAG 链路、评测与成本优化等高价值信息。
...
1. **projectExperienceScore（0-40，项目经验）**：AI/Agent 相关项目是否真实可追问；是否体现可落地的方案设计与迭代；是否有量化结果（命中率、准确率、响应时延、成本优化等）。
   - 31-40：项目深度强、证据充分、有量化结果
   - 18-30：有项目但深度不足或证据偏弱
   - 0-17：关键词堆砌、描述空泛或明显不匹配

2. **skillMatchScore（0-20，技能匹配）**：AI/Agent 校招核心能力覆盖（LLM 应用、Agent 工作流、RAG、工程落地 Python/TypeScript、评测与可靠性意识）；是否有项目或实习佐证。
...
4. **structureClarityScore（0-15，结构清晰度）**：简历层次是否清晰、模块划分是否易读；技术名词是否规范（LLM、RAG、Agent、Embedding、Prompt、Evaluation）；是否存在概念混淆。
   - 12-15：结构清晰、排版规范
   - 6-11：可读但层次或规范一般
   - 0-5：混乱或大量不规范表达
...
3. 若缺少 AI/Agent 核心项（LLM 应用基础 / Agent 或 RAG 实践 / 项目实践）中的 2 项及以上，scoreDetail 五项之和不得高于 65。
