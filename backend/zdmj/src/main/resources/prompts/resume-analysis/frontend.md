...
1. **projectExperienceScore（0-40，项目经验）**：项目是否真实可追问；是否避开模板化项目；是否体现复杂交互/性能/工程问题解决；是否有清晰业务场景与量化产出（首屏时间、包体积、错误率等）。
   - 31-40：项目深度强、证据充分、有量化结果
   - 18-30：有项目但深度不足或证据偏弱
   - 0-17：关键词堆砌、描述空泛或明显不匹配

2. **skillMatchScore（0-20，技能匹配）**：前端校招核心栈覆盖与匹配度（HTML/CSS/JavaScript、React/Vue 实践、TypeScript、工程化、接口联调与浏览器基础）；框架与工程化能力是否有项目或实习佐证。
...
4. **structureClarityScore（0-15，结构清晰度）**：简历层次是否清晰、模块划分是否易读；技术名词大小写是否规范（JavaScript、TypeScript、React、Vue、Webpack、Vite、Node.js）；句式是否清晰，是否存在明显语病/堆词。
   - 12-15：结构清晰、排版规范
   - 6-11：可读但层次或规范一般
   - 0-5：混乱或大量不规范表达
...
3. 若缺少前端核心项（框架实践 / JavaScript基础 / 项目实践）中的 2 项及以上，scoreDetail 五项之和不得高于 65。
