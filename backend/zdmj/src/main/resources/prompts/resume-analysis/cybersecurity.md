...
1. **安全方案合理性**：识别并纠正夸大或不符合实际的安全描述。
2. **业务场景融合**：描述必须遵循「风险场景 + 检测/验证/处置 + 结果」的模式。
3. **表达精炼度**：单条描述建议不超过两行。动词开头，删除冗余词汇。
4. **深度安全点**：优先挖掘漏洞分析、日志研判、应急响应、安全基线等高价值信息。
...
1. **projectExperienceScore（0-40，项目经验）**：安全相关项目是否真实可追问；是否体现漏洞发现、防护或应急处置闭环；是否有量化结果（漏洞修复率、响应时长、告警误报率等）。
   - 31-40：项目深度强、证据充分、有量化结果
   - 18-30：有项目但深度不足或证据偏弱
   - 0-17：关键词堆砌、描述空泛或明显不匹配

2. **skillMatchScore（0-20，技能匹配）**：网络安全校招核心能力覆盖（网络与系统安全基础、漏洞分析、安全监控与日志、应急响应、常用安全工具）；是否有项目或实习佐证。
...
4. **structureClarityScore（0-15，结构清晰度）**：简历层次是否清晰、模块划分是否易读；技术名词是否规范（CVE、CVSS、WAF、SIEM、SOC 等）；句式是否清晰，是否存在明显语病/堆词。
   - 12-15：结构清晰、排版规范
   - 6-11：可读但层次或规范一般
   - 0-5：混乱或大量不规范表达
...
3. 若缺少安全核心项（安全基础 / 漏洞或防护实践 / 项目实践）中的 2 项及以上，scoreDetail 五项之和不得高于 65。
