...
1. 七维与 `strengths` 只基于输入岗位信息；未出现的技术要求不得写入这些字段。
...
- `summary` 需同时覆盖：岗位定位、前端核心门槛、协作与交付要求、信息完备度判断；不得给出候选人行动建议。
- `strengths` 输出 3～6 条，每条需包含“能力点 + 岗位文本依据（关键词/职责/要求）”。
- `missingSkills` 输出 0～5 条，语义是补充要求。仅列硬门槛：如 JD 未写 HTTP/浏览器基础、Git、CSS 布局可列；不得列低代码平台、可视化引擎等进阶项。
